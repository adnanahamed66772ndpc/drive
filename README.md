...
Pentaract is aimed to take as small disk space as possible.

## Auto-deploy to VPS (git push = deploy)

Set GitHub Actions secrets once, then **every push to main** auto-deploys to your VPS. See [GitHub Secrets checklist](docs/github-secrets-checklist.md) for required secrets (VPS host, SSH key, superuser email/password, SECRET_KEY, etc.). So it does not need any code interpreter/platform to run. The whole app is just several megabytes in size. It also uses Postgres as a database and we try our best to economy space by not creating unneeded fields and tables and to wisely pick proper datatypes.
